const path = require("path");
const fs = require("fs");
const express = require("express");
const expressLayouts = require("express-ejs-layouts");
const helmet = require("helmet");
const session = require("express-session");
const SQLiteStore = require("connect-sqlite3")(session);
const multer = require("multer");
const crypto = require("crypto");
const mime = require("mime-types");

const { db, initDb } = require("./src/db");
const {
  requireAuth,
  attachUser,
  hashPassword,
  verifyPassword,
  normalizeEmail,
} = require("./src/auth");
const { ideaStatusLabel, isValidIdeaStatus } = require("./src/ideas");

const PORT = process.env.PORT || 3000;
const UPLOAD_DIR = path.join(__dirname, "uploads");

if (!fs.existsSync(UPLOAD_DIR)) fs.mkdirSync(UPLOAD_DIR, { recursive: true });

initDb();

const app = express();
app.set("view engine", "ejs");
app.set("views", path.join(__dirname, "src", "views"));
app.set("layout", path.join("partials", "layout"));

app.use(expressLayouts);

app.use(
  helmet({
    contentSecurityPolicy: false, // keep simple for local assets
  })
);
app.use(express.urlencoded({ extended: false }));
app.use(express.json());
app.use(
  session({
    store: new SQLiteStore({ db: "sessions.sqlite", dir: path.join(__dirname, "data") }),
    secret: process.env.SESSION_SECRET || "dev-secret-change-me",
    resave: false,
    saveUninitialized: false,
    cookie: { sameSite: "lax" },
  })
);

app.use("/public", express.static(path.join(__dirname, "public")));
app.use("/uploads", express.static(UPLOAD_DIR));

app.use(attachUser);

const upload = multer({
  storage: multer.diskStorage({
    destination: (_req, _file, cb) => cb(null, UPLOAD_DIR),
    filename: (_req, file, cb) => {
      const ext = mime.extension(file.mimetype) || "bin";
      const rand = crypto.randomBytes(6).toString("hex");
      cb(null, `${Date.now()}-${rand}.${ext}`);
    },
  }),
  limits: { fileSize: 50 * 1024 * 1024 },
  fileFilter: (_req, file, cb) => {
    const okImages = ["image/png", "image/jpeg", "image/webp", "image/gif"];
    const okVideos = ["video/mp4", "video/webm", "video/quicktime"];
    const ok = [...okImages, ...okVideos].includes(file.mimetype);
    cb(ok ? null : new Error("Only image/video uploads are allowed."), ok);
  },
});

function setFlash(req, type, message) {
  if (!req.session) return;
  req.session.flash = { type, message };
}

function consumeFlash(req) {
  const flash = req.session.flash;
  delete req.session.flash;
  return flash;
}

function wantsJson(req) {
  return req.path.startsWith("/api/") || req.headers.accept?.includes("application/json");
}

app.use((req, res, next) => {
  res.locals.user = req.user || null;
  res.locals.flash = consumeFlash(req);
  res.locals.path = req.path;
  next();
});

app.get("/", (req, res) => {
  const ideas = db
    .prepare(
      `
      SELECT
        i.*,
        u.name as authorName,
        (SELECT COUNT(*) FROM votes v WHERE v.ideaId = i.id) as voteCount,
        (SELECT COUNT(*) FROM comments c WHERE c.ideaId = i.id) as commentCount
      FROM ideas i
      JOIN users u ON u.id = i.userId
      ORDER BY i.createdAt DESC
      `
    )
    .all();
  res.render("home", { title: "Home", ideas, ideaStatusLabel });
});

app.get("/about", (_req, res) => res.render("about", { title: "About" }));
app.get("/help", (_req, res) => res.render("help", { title: "Help & FAQ" }));

app.get("/achievers", (_req, res) => {
  const topIdeas = db
    .prepare(
      `
      SELECT i.*, u.name as authorName,
        (SELECT COUNT(*) FROM votes v WHERE v.ideaId = i.id) as voteCount
      FROM ideas i
      JOIN users u ON u.id = i.userId
      ORDER BY voteCount DESC, i.createdAt DESC
      LIMIT 10
      `
    )
    .all();

  const topUsers = db
    .prepare(
      `
      SELECT u.id, u.name, u.email,
        (SELECT COUNT(*) FROM ideas i WHERE i.userId = u.id) as ideaCount,
        (SELECT COUNT(*) FROM votes v JOIN ideas i ON i.id = v.ideaId WHERE i.userId = u.id) as receivedVotes
      FROM users u
      ORDER BY receivedVotes DESC, ideaCount DESC, u.createdAt ASC
      LIMIT 10
      `
    )
    .all();

  res.render("achievers", { title: "Achievers", topIdeas, topUsers, ideaStatusLabel });
});

app.get("/auth", (req, res) => {
  if (req.user) return res.redirect("/");
  res.render("auth", { title: "Sign in" });
});

app.post("/auth/signup", (req, res) => {
  const name = (req.body.name || "").trim();
  const email = normalizeEmail(req.body.email);
  const password = req.body.password || "";

  if (!name || !email || password.length < 6) {
    setFlash(req, "danger", "Enter name, valid email, and password (min 6 chars).");
    return res.redirect("/auth");
  }

  const existing = db.prepare("SELECT id FROM users WHERE email = ?").get(email);
  if (existing) {
    setFlash(req, "danger", "That email is already registered. Please sign in.");
    return res.redirect("/auth");
  }

  const isFirstUser = db.prepare("SELECT COUNT(*) as c FROM users").get().c === 0;
  const role = isFirstUser ? "admin" : "user";
  const now = new Date().toISOString();

  const info = db
    .prepare("INSERT INTO users (name, email, passwordHash, role, createdAt) VALUES (?, ?, ?, ?, ?)")
    .run(name, email, hashPassword(password), role, now);

  req.session.userId = info.lastInsertRowid;
  setFlash(req, "success", "Welcome! Your account is ready.");
  res.redirect("/");
});

app.post("/auth/signin", (req, res) => {
  const email = normalizeEmail(req.body.email);
  const password = req.body.password || "";

  const user = db.prepare("SELECT * FROM users WHERE email = ?").get(email);
  if (!user || !verifyPassword(password, user.passwordHash)) {
    setFlash(req, "danger", "Invalid email or password.");
    return res.redirect("/auth");
  }

  req.session.userId = user.id;
  setFlash(req, "success", `Welcome back, ${user.name}.`);
  res.redirect("/");
});

app.post("/auth/signout", (req, res) => {
  req.session.destroy(() => res.redirect("/"));
});

app.get("/ideas/new", requireAuth, (req, res) => {
  res.render("idea_new", { title: "Submit idea" });
});

app.post("/ideas", requireAuth, upload.fields([{ name: "attachment", maxCount: 1 }, { name: "video", maxCount: 1 }]), (req, res) => {
  const title = (req.body.title || "").trim();
  const description = (req.body.description || "").trim();
  const attachment = req.files?.attachment?.[0];
  const video = req.files?.video?.[0];
  const attachmentPath = attachment ? `/uploads/${attachment.filename}` : null;
  const videoPath = video ? `/uploads/${video.filename}` : null;

  if (!title || description.length < 10) {
    setFlash(req, "danger", "Title is required and description must be at least 10 characters.");
    return res.redirect("/ideas/new");
  }

  const now = new Date().toISOString();
  const status = "inprogress";

  const info = db
    .prepare(
      "INSERT INTO ideas (userId, title, description, attachmentPath, videoPath, status, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?)"
    )
    .run(req.user.id, title, description, attachmentPath, videoPath, status, now, now);

  setFlash(req, "success", "Idea submitted. You can track status in My Ideas.");
  res.redirect(`/ideas/${info.lastInsertRowid}`);
});

app.post("/api/enhance-description", requireAuth, async (req, res) => {
  const input = String(req.body?.text || "").trim();
  if (input.length < 5) return res.status(400).json({ error: "Please enter more text to enhance." });

  const provider = String(process.env.AI_PROVIDER || "").trim().toLowerCase() || "ollama";

  try {
    const prompt =
      "Improve the following idea description. Return ONLY the improved description text. Keep it concise, clear, and structured with short paragraphs and optional bullet points.\n\n" +
      input;

    if (provider === "openai") {
      const apiKey = process.env.OPENAI_API_KEY;
      if (!apiKey) return res.status(501).json({ error: "OpenAI is not configured. Set OPENAI_API_KEY." });

      const resp = await fetch("https://api.openai.com/v1/chat/completions", {
        method: "POST",
        headers: {
          "content-type": "application/json",
          authorization: `Bearer ${apiKey}`,
        },
        body: JSON.stringify({
          model: process.env.OPENAI_MODEL || "gpt-4o-mini",
          temperature: 0.3,
          messages: [
            {
              role: "system",
              content:
                "You improve user-submitted ideas. Return ONLY the improved description text. Keep it concise, clear, and structured with short paragraphs and optional bullet points.",
            },
            { role: "user", content: input },
          ],
        }),
      });

      if (!resp.ok) {
        const t = await resp.text();
        return res.status(502).json({ error: `OpenAI error: ${t.slice(0, 300)}` });
      }
      const data = await resp.json();
      const text = data?.choices?.[0]?.message?.content?.trim();
      if (!text) return res.status(502).json({ error: "OpenAI returned no text." });
      return res.json({ text });
    }

    if (provider === "ollama") {
      const baseUrl = process.env.OLLAMA_URL || "http://localhost:11434";
      const model = process.env.OLLAMA_MODEL || "llama3.1";

      const resp = await fetch(`${baseUrl}/api/generate`, {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
          model,
          prompt,
          stream: false,
          options: {
            temperature: 0.3,
          },
        }),
      });

      if (!resp.ok) {
        const t = await resp.text();
        return res.status(502).json({
          error:
            `Ollama error: ${t.slice(0, 300)}. ` +
            "Make sure Ollama is running and the model is pulled (e.g. `ollama pull llama3.1`).",
        });
      }
      const data = await resp.json();
      const text = String(data?.response || "").trim();
      if (!text) return res.status(502).json({ error: "Ollama returned no text." });
      return res.json({ text });
    }

    // Fallback (no external AI): basic cleanup + structure
    const cleaned = input
      .replace(/\r\n/g, "\n")
      .replace(/[ \t]+\n/g, "\n")
      .replace(/\n{3,}/g, "\n\n")
      .trim();

    const text =
      "Problem / Context:\n" +
      cleaned +
      "\n\nProposed solution:\n- \n\nExpected impact:\n- \n\nNotes:\n- ";
    return res.json({ text });
  } catch (e) {
    return res.status(500).json({ error: e?.message || "Failed to enhance text." });
  }
});

function normalizeTitle(s) {
  return String(s || "")
    .toLowerCase()
    .replace(/[^a-z0-9\s]/g, " ")
    .replace(/\s+/g, " ")
    .trim();
}

function trigramSet(s) {
  const t = `  ${normalizeTitle(s)}  `;
  const set = new Set();
  for (let i = 0; i < t.length - 2; i++) set.add(t.slice(i, i + 3));
  return set;
}

function jaccard(a, b) {
  if (!a.size && !b.size) return 1;
  let inter = 0;
  for (const x of a) if (b.has(x)) inter++;
  const uni = a.size + b.size - inter;
  return uni ? inter / uni : 0;
}

app.get("/api/ideas/duplicate-check", requireAuth, (req, res) => {
  const title = String(req.query.title || "").trim();
  if (title.length < 3) return res.json({ percent: 0, bestMatch: null });

  const titles = db.prepare("SELECT id, title FROM ideas ORDER BY createdAt DESC LIMIT 500").all();
  if (!titles.length) return res.json({ percent: 0, bestMatch: null });

  const base = trigramSet(title);
  let best = { percent: 0, id: null, title: null };

  for (const row of titles) {
    const score = jaccard(base, trigramSet(row.title));
    const percent = Math.round(score * 100);
    if (percent > best.percent) best = { percent, id: row.id, title: row.title };
  }

  return res.json({ percent: best.percent, bestMatch: best.id ? { id: best.id, title: best.title } : null });
});

app.get("/ideas/:id", (req, res) => {
  const id = Number(req.params.id);
  const idea = db
    .prepare(
      `
      SELECT i.*, u.name as authorName,
        (SELECT COUNT(*) FROM votes v WHERE v.ideaId = i.id) as voteCount
      FROM ideas i
      JOIN users u ON u.id = i.userId
      WHERE i.id = ?
      `
    )
    .get(id);

  if (!idea) return res.status(404).render("not_found", { title: "Not found" });

  const comments = db
    .prepare(
      `
      SELECT c.*, u.name as authorName
      FROM comments c
      JOIN users u ON u.id = c.userId
      WHERE c.ideaId = ?
      ORDER BY c.createdAt ASC
      `
    )
    .all(id);

  const hasVoted =
    req.user &&
    !!db.prepare("SELECT 1 FROM votes WHERE ideaId = ? AND userId = ?").get(id, req.user.id);

  res.render("idea_view", { title: idea.title, idea, comments, hasVoted, ideaStatusLabel });
});

app.post("/ideas/:id/vote", requireAuth, (req, res) => {
  const id = Number(req.params.id);
  const idea = db.prepare("SELECT * FROM ideas WHERE id = ?").get(id);
  if (!idea) return res.status(404).render("not_found", { title: "Not found" });

  const existing = db.prepare("SELECT 1 FROM votes WHERE ideaId = ? AND userId = ?").get(id, req.user.id);
  const now = new Date().toISOString();

  if (existing) {
    db.prepare("DELETE FROM votes WHERE ideaId = ? AND userId = ?").run(id, req.user.id);
    setFlash(req, "info", "Vote removed.");
  } else {
    db.prepare("INSERT INTO votes (ideaId, userId, createdAt) VALUES (?, ?, ?)").run(id, req.user.id, now);

    if (idea.userId !== req.user.id) {
      db.prepare(
        "INSERT INTO notifications (userId, type, message, link, isRead, createdAt) VALUES (?, ?, ?, ?, 0, ?)"
      ).run(
        idea.userId,
        "vote",
        `${req.user.name} voted for your idea: "${idea.title}"`,
        `/ideas/${id}`,
        now
      );
    }
    setFlash(req, "success", "Thanks for voting!");
  }

  res.redirect(req.get("referer") || `/ideas/${id}`);
});

app.post("/api/ideas/:id/vote", requireAuth, (req, res) => {
  const id = Number(req.params.id);
  const idea = db.prepare("SELECT * FROM ideas WHERE id = ?").get(id);
  if (!idea) return res.status(404).json({ error: "Not found" });

  const existing = db.prepare("SELECT 1 FROM votes WHERE ideaId = ? AND userId = ?").get(id, req.user.id);
  const now = new Date().toISOString();

  let hasVoted = false;
  if (existing) {
    db.prepare("DELETE FROM votes WHERE ideaId = ? AND userId = ?").run(id, req.user.id);
    hasVoted = false;
  } else {
    db.prepare("INSERT INTO votes (ideaId, userId, createdAt) VALUES (?, ?, ?)").run(id, req.user.id, now);
    hasVoted = true;

    if (idea.userId !== req.user.id) {
      db.prepare(
        "INSERT INTO notifications (userId, type, message, link, isRead, createdAt) VALUES (?, ?, ?, ?, 0, ?)"
      ).run(idea.userId, "vote", `${req.user.name} voted for your idea: \"${idea.title}\"`, `/ideas/${id}`, now);
    }
  }

  const voteCount = db.prepare("SELECT COUNT(*) as c FROM votes WHERE ideaId = ?").get(id).c;
  return res.json({ ideaId: id, voteCount, hasVoted });
});

app.post("/ideas/:id/comments", requireAuth, (req, res) => {
  const id = Number(req.params.id);
  const content = (req.body.content || "").trim();
  const parentCommentId = req.body.parentCommentId ? Number(req.body.parentCommentId) : null;
  if (content.length < 2) {
    setFlash(req, "danger", "Comment is too short.");
    return res.redirect(`/ideas/${id}`);
  }

  const idea = db.prepare("SELECT * FROM ideas WHERE id = ?").get(id);
  if (!idea) return res.status(404).render("not_found", { title: "Not found" });

  const now = new Date().toISOString();
  db.prepare("INSERT INTO comments (ideaId, userId, parentCommentId, content, createdAt) VALUES (?, ?, ?, ?, ?)")
    .run(id, req.user.id, parentCommentId, content, now);

  if (idea.userId !== req.user.id) {
    db.prepare(
      "INSERT INTO notifications (userId, type, message, link, isRead, createdAt) VALUES (?, ?, ?, ?, 0, ?)"
    ).run(
      idea.userId,
      "comment",
      `${req.user.name} commented on your idea: "${idea.title}"`,
      `/ideas/${id}`,
      now
    );
  }

  setFlash(req, "success", "Comment posted.");
  res.redirect(`/ideas/${id}#comments`);
});

app.get("/api/ideas/:id/comments", (req, res) => {
  const id = Number(req.params.id);
  const idea = db.prepare("SELECT id FROM ideas WHERE id = ?").get(id);
  if (!idea) return res.status(404).json({ error: "Not found" });

  const rows = db
    .prepare(
      `
      SELECT c.*, u.name as authorName
      FROM comments c
      JOIN users u ON u.id = c.userId
      WHERE c.ideaId = ?
      ORDER BY c.createdAt ASC
      `
    )
    .all(id);

  return res.json({ ideaId: id, comments: rows });
});

app.post("/api/ideas/:id/comments", requireAuth, (req, res) => {
  const id = Number(req.params.id);
  const content = String(req.body?.content || "").trim();
  const parentCommentId = req.body?.parentCommentId ? Number(req.body.parentCommentId) : null;

  if (content.length < 2) return res.status(400).json({ error: "Comment is too short." });
  const idea = db.prepare("SELECT * FROM ideas WHERE id = ?").get(id);
  if (!idea) return res.status(404).json({ error: "Not found" });

  if (parentCommentId) {
    const parent = db.prepare("SELECT id, ideaId FROM comments WHERE id = ?").get(parentCommentId);
    if (!parent || parent.ideaId !== id) return res.status(400).json({ error: "Invalid parent comment." });
  }

  const now = new Date().toISOString();
  const info = db
    .prepare("INSERT INTO comments (ideaId, userId, parentCommentId, content, createdAt) VALUES (?, ?, ?, ?, ?)")
    .run(id, req.user.id, parentCommentId, content, now);

  if (idea.userId !== req.user.id) {
    db.prepare(
      "INSERT INTO notifications (userId, type, message, link, isRead, createdAt) VALUES (?, ?, ?, ?, 0, ?)"
    ).run(
      idea.userId,
      "comment",
      `${req.user.name} commented on your idea: \"${idea.title}\"`,
      `/ideas/${id}`,
      now
    );
  }

  const commentCount = db.prepare("SELECT COUNT(*) as c FROM comments WHERE ideaId = ?").get(id).c;
  return res.json({ ideaId: id, commentId: info.lastInsertRowid, commentCount });
});

app.get("/my-ideas", requireAuth, (req, res) => {
  const ideas = db
    .prepare(
      `
      SELECT
        i.*,
        (SELECT COUNT(*) FROM votes v WHERE v.ideaId = i.id) as voteCount,
        (SELECT COUNT(*) FROM comments c WHERE c.ideaId = i.id) as commentCount
      FROM ideas i
      WHERE i.userId = ?
      ORDER BY i.createdAt DESC
      `
    )
    .all(req.user.id);

  res.render("my_ideas", { title: "My Ideas", ideas, ideaStatusLabel });
});

app.get("/profile", requireAuth, (req, res) => res.render("profile", { title: "Profile" }));

app.get("/inbox", requireAuth, (req, res) => {
  const notifications = db
    .prepare("SELECT * FROM notifications WHERE userId = ? ORDER BY createdAt DESC LIMIT 50")
    .all(req.user.id);
  res.render("inbox", { title: "Inbox", notifications });
});

app.post("/inbox/mark-read", requireAuth, (req, res) => {
  db.prepare("UPDATE notifications SET isRead = 1 WHERE userId = ?").run(req.user.id);
  res.redirect("/inbox");
});

app.get("/admin/ideas", requireAuth, (req, res) => {
  if (req.user.role !== "admin") return res.status(403).render("forbidden", { title: "Forbidden" });

  const ideas = db
    .prepare(
      `
      SELECT i.*, u.name as authorName
      FROM ideas i
      JOIN users u ON u.id = i.userId
      ORDER BY i.createdAt DESC
      `
    )
    .all();

  res.render("admin_ideas", { title: "Admin: Ideas", ideas, ideaStatusLabel });
});

app.get("/admin/dashboard", requireAuth, (req, res) => {
  if (req.user.role !== "admin") return res.status(403).render("forbidden", { title: "Forbidden" });

  const userCount = db.prepare("SELECT COUNT(*) as c FROM users").get().c;
  const ideaCount = db.prepare("SELECT COUNT(*) as c FROM ideas").get().c;
  const inprogressCount = db.prepare("SELECT COUNT(*) as c FROM ideas WHERE status = 'inprogress'").get().c;
  const approvedCount = db.prepare("SELECT COUNT(*) as c FROM ideas WHERE status = 'approved'").get().c;
  const closedCount = db.prepare("SELECT COUNT(*) as c FROM ideas WHERE status = 'closed'").get().c;

  res.render("admin_dashboard", {
    title: "Admin: Dashboard",
    stats: { userCount, ideaCount, inprogressCount, approvedCount, closedCount },
  });
});

app.post("/admin/ideas/:id/status", requireAuth, (req, res) => {
  if (req.user.role !== "admin") return res.status(403).render("forbidden", { title: "Forbidden" });
  const id = Number(req.params.id);
  const status = (req.body.status || "").trim();
  const closeJustification = String(req.body.closeJustification || "").trim();
  if (!isValidIdeaStatus(status)) {
    setFlash(req, "danger", "Invalid status.");
    return res.redirect("/admin/ideas");
  }
  const idea = db.prepare("SELECT * FROM ideas WHERE id = ?").get(id);
  if (!idea) return res.status(404).render("not_found", { title: "Not found" });

  const now = new Date().toISOString();
  if (status === "closed" && closeJustification.length < 5) {
    setFlash(req, "danger", "Closing requires a short justification (min 5 chars).");
    return res.redirect("/admin/ideas");
  }

  db.prepare("UPDATE ideas SET status = ?, closeJustification = ?, updatedAt = ? WHERE id = ?").run(
    status,
    status === "closed" ? closeJustification : null,
    now,
    id
  );

  if (idea.userId) {
    db.prepare(
      "INSERT INTO notifications (userId, type, message, link, isRead, createdAt) VALUES (?, ?, ?, ?, 0, ?)"
    ).run(
      idea.userId,
      "status",
      `Your idea "${idea.title}" is now ${ideaStatusLabel(status)}.`,
      `/ideas/${id}`,
      now
    );
  }

  setFlash(req, "success", "Status updated.");
  res.redirect("/admin/ideas");
});

app.use((_req, res) => res.status(404).render("not_found", { title: "Not found" }));

app.use((err, req, res, _next) => {
  console.error(err);
  if (wantsJson(req)) {
    return res.status(500).json({ error: err.message || "Something went wrong." });
  }
  setFlash(req, "danger", err.message || "Something went wrong.");
  return res.redirect(req.get("referer") || "/");
});

app.listen(PORT, () => {
  console.log(`IMS Ideas running at http://localhost:${PORT}`);
});

